...
# Sources: ACR DIR 2023 national medians, AAPM guidelines
# These are approximate — update with current published values

BENCHMARK_VERSION = "ACR DIR 2023–2024"
...
def analyze_facility(df: pd.DataFrame, facility_name: str = "Facility") -> dict:
    ...
    for region in ["Head", "Chest", "Abdomen-Pelvis", 
        ...
        stats = compute_percentiles(region_df['dlp'])
    ...
    if 'is_pediatric' in df.columns:
        ...
        for region in ["Head", "Abdomen-Pelvis"]:
            ...
            for age_grp in PEDS_AGE_GROUPS:
                ...
                stats = compute_percentiles(age_df['dlp'])
...
def print_report(results: dict):
    ...
    print(f"  {'Region':<25} {'N':>5} {'P25':>8} {'P50':>8} {'P75':>8} {'Status':<18}")
    print(f"  {'-'*23:<25} {'---':>5} {'---':>8} {'---':>8} {'---':>8} {'-'*16:<18}")
    ...
    for region, data in results["adult"].items():
        ...
        print(f"  {region:<25} {s['n']:>5} {s['p25']:>8.0f} {s['p50']:>8.0f} "
